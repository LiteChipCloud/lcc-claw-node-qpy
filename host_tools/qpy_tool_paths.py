...
def candidate_user_tool_paths(exe_name: str) -> List[Path]:
    name = exe_name.strip()
    if not name:
        return []
    ...
    home = Path.home()
    direct_candidates = [
        home / ".codex" / "skills" / "quecpython-dev" / "review" / "force_repro_tools" / name,
    ]

    glob_patterns = [
        ".vscode/extensions/*/scripts/%s" % name,
        ".vscode-insiders/extensions/*/scripts/%s" % name,
    ]
    ...
    for pattern in glob_patterns:
        try:
            discovered.extend(home.glob(pattern))
        except Exception:
            continue
    return _dedup_paths(discovered)
...
def resolve_windows_exe(
    ...
    candidates.extend(candidate_user_tool_paths(exe_name))
