...
def repl_send_lines(
    ...
    here_payload = "\n" + payload + "\n"
    # Do not force modem control lines here. On some Quectel Windows drivers,
    # DTR/RTS toggling from SSH-hosted PowerShell sessions breaks COM open.
    ps = (
        "$sp=$null; try {"
...
def read_remote_size_repl(port: str, baud: int, remote_path: str, timeout: int) -> int:
    path_qpy = single_quote_qpy(normalize_remote_path(remote_path))
    code = "\n".join(
        [
            "import uos",
            "_size = -1",
            "try:",
            " _size = uos.stat('%s')[6]" % path_qpy,
            "except Exception:",
            " _size = -1",
            "print('remote_size', _size)",
        ]
    )
    raw = repl_send_lines(
        port,
        baud,
        ["exec(%s)" % json.dumps(code)],
        timeout=max(10, timeout),
        line_delay_ms=45,
        settle_ms=700,
    )
    m = re.search(r"remote_size\s+(-?\d+)", raw)
    return int(m.group(1)) if m else -1


def select_push_policy(local_size: int) -> Dict[str, int]:
    if local_size <= 2048:
        return {
            "chunk_size": 96,
            "append_batch": 8,
...
def run_push_repl(
    ...
    policy = select_push_policy(len(data))
    chunks = chunk_hex(data, chunk_size=int(policy["chunk_size"]))
    remote_path = join_remote_path(remote_dir, remote_name)
    tmp_remote_name = "%s.upload_%d.tmp" % (remote_name, int(time.time() * 1000))
    bak_remote_name = "%s.rollback_%d.bak" % (remote_name, int(time.time() * 1000))
    remote_dir_qpy = single_quote_qpy(normalize_remote_path(remote_dir))
    remote_path_qpy = single_quote_qpy(remote_path)
    tmp_path_qpy = single_quote_qpy(join_remote_path(remote_dir, tmp_remote_name))
    remote_name_qpy = single_quote_qpy(remote_name)
    tmp_name_qpy = single_quote_qpy(tmp_remote_name)
    bak_path_qpy = single_quote_qpy(join_remote_path(remote_dir, bak_remote_name))
    bak_name_qpy = single_quote_qpy(bak_remote_name)
    append_batch = int(policy["append_batch"])
    ...
    has_error = any(x in raw for x in ["ERR:", "Traceback", "ParserError", "At line:"])
    if ("push_init_ok" not in raw) or has_error:
        observed_init_size = read_remote_size_repl(port, baud, join_remote_path(remote_dir, tmp_remote_name), timeout)
        combined_raw.append("[init_size_check observed=%d]" % observed_init_size)
        if observed_init_size != 0:
            return {
                "ok": False,
                "raw": "\n".join(combined_raw),
                "local_size": len(data),
                ...
                "tmp_remote_name": tmp_remote_name,
                "backup_remote_name": bak_remote_name,
                "chunks": len(chunks),
                "push_policy": policy,
                "backend": "repl",
            }
    written = 0
    expected = len(data)
    for index in range(0, len(chunks), append_batch):
        batch = chunks[index : index + append_batch]
        batch_size = sum(len(bytes.fromhex(hx)) for hx in batch)
        prior_written = written
        expected_written = written + batch_size
        batch_lines: List[str] = [
            "import gc,ubinascii",
            "gc.collect()",
            "f=open('%s','ab')" % tmp_path_qpy,
        ]
        for hx in batch:
            batch_lines.append("f.write(ubinascii.unhexlify('%s'))" % hx)
        batch_lines.extend(
            [
                "f.close()",
                "gc.collect()",
                "print('push_batch_ok %d')" % expected_written,
            ]
        )
        success = False
        for attempt in range(0, int(policy["batch_retries"]) + 1):
            raw = repl_send_lines(
                port,
                baud,
                batch_lines,
                timeout=max(10, timeout),
                line_delay_ms=int(policy["line_delay_ms"]),
                settle_ms=int(policy["settle_ms"]),
            )
            combined_raw.append(raw)
            has_error = any(x in raw for x in ["ERR:", "Traceback", "ParserError", "At line:"])
            success_marker = "push_batch_ok %d" % expected_written
            if (success_marker in raw) and (not has_error):
                written = expected_written
                success = True
                break
            observed_tmp_size = read_remote_size_repl(
                port,
                baud,
                join_remote_path(remote_dir, tmp_remote_name),
                timeout,
            )
            combined_raw.append(
                "[batch_size_check attempt=%d observed=%d expected=%d prior=%d]"
                % (attempt + 1, observed_tmp_size, expected_written, prior_written)
            )
            if observed_tmp_size == expected_written:
                written = expected_written
                success = True
                break
            if observed_tmp_size not in {-1, prior_written}:
                return {
                    "ok": False,
                    "raw": "\n".join(combined_raw),
                    "local_size": expected,
                    "bytes_written": observed_tmp_size,
                    "remote_size": observed_tmp_size,
                    "remote_path": remote_path,
                    "tmp_remote_name": tmp_remote_name,
                    "backup_remote_name": bak_remote_name,
                    "chunks": len(chunks),
            ...
            if attempt < int(policy["batch_retries"]):
                time.sleep((120 + (attempt * 180)) / 1000.0)
        if not success:
            return {
                "ok": False,
                "raw": "\n".join(combined_raw),
        ...
        if written < expected:
            time.sleep(int(policy["inter_batch_sleep_ms"]) / 1000.0)
    ...
    final_code = "\n".join(
        [
            ...
            "gc.collect()",
            "_entries=uos.listdir('%s')" % remote_dir_qpy,
            "_bak_name='%s'" % bak_name_qpy,
            "_bak_path='%s'" % bak_path_qpy,
            "_tmp_path='%s'" % tmp_path_qpy,
            "_live_name='%s'" % remote_name_qpy,
            "_live_path='%s'" % remote_path_qpy,
            "if _bak_name in _entries:",
            " uos.remove(_bak_path)",
            "if _live_name in _entries:",
            " uos.rename(_live_path, _bak_path)",
            "_ok=False",
            "try:",
            " uos.rename(_tmp_path, _live_path)",
            " _ok=True",
            "except Exception as e:",
            " _restore_entries = uos.listdir('%s')" % remote_dir_qpy,
            " if (_bak_name in _restore_entries) and (_live_name not in _restore_entries):",
            "  uos.rename(_bak_path, _live_path)",
            " print('push_commit_err', e)",
            "if _ok:",
            " _cleanup_entries = uos.listdir('%s')" % remote_dir_qpy,
            " if _bak_name in _cleanup_entries:",
            "  uos.remove(_bak_path)",
            " gc.collect()",
            " print('push_ok', %d, uos.stat(_live_path)[6])" % len(data),
        ]
    )
    final_lines = ["exec(%s)" % json.dumps(final_code)]
    remote_size = -1
    ok = False
    for attempt in range(0, 3):
        ...
        if (not has_error) and m:
            written = int(m.group(1))
            remote_size = int(m.group(2))
            ok = written == expected and remote_size == expected
            if ok:
                break
        remote_size = read_remote_size_repl(port, baud, remote_path, timeout)
        tmp_size = read_remote_size_repl(port, baud, join_remote_path(remote_dir, tmp_remote_name), timeout)
        combined_raw.append(
            "[commit_size_check attempt=%d live=%d tmp=%d expected=%d]"
            % (attempt + 1, remote_size, tmp_size, expected)
        )
        if remote_size == expected and tmp_size == -1:
            written = expected
            ok = True
            break
        if attempt < 2 and tmp_size == expected and remote_size != expected:
            time.sleep((220 + (attempt * 220)) / 1000.0)
            continue
        break
    return {
        "ok": ok,
...
def run_repl_op(port: str, baud: int, lines: List[str], success_marker: str, timeout: int) -> Dict[str, Any]:
    raw = repl_send_lines(port, baud, lines, timeout=max(10, timeout))
    has_error = any(x in raw for x in ["ERR:", "Traceback", "ParserError", "At line:"])
    ok = (success_marker in raw) and (not has_error)
