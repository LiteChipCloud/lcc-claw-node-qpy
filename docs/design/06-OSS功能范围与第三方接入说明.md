...
4. 主动上报 `node.event(event=\"agent.request\")`。

## 2.3 主动上报能力

首版主动上报至少包含：

1. `agent.request`：设备主动把业务告警、人工介入请求、业务事件说明送入 Gateway agent。
2. 可选原始 `node.event` 扩展：`heartbeat / telemetry / alert / lifecycle`，仅面向自定义 Gateway 消费器。
3. `alert` 业务语义仍然成立，例如：
   - 网络反复重连
   - SIM 未注册
   - 业务命令执行失败达到阈值
4. 但在官方 Gateway 基线下，这类 `alert` 默认应通过 `agent.request` 承载，而不是假设 Gateway 会直接显示自定义 `alert` 事件。
...
7. 假设官方 Gateway 会自动消费任意自定义 `node.event` 名称。
...
5. 设备可主动把业务告警上送到 Gateway agent。
6. 如果用户自己扩展了 Gateway，也可以进一步接收原始 `heartbeat / telemetry / lifecycle / alert` 事件。
